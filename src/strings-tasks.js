...
function countVowels(str) {
  ...
  for (let i = 0; i < newArray.length; i += 1) {
    if (
      ['a', 'e', 'i', 'o', 'u', 'y'].indexOf(newArray[i].toLowerCase()) !== -1
    )
      count += 1;
  }
  return count;
...
function isPalindrome(str) {
  const str1 = str.toLowerCase().replace(/[^\w$]/gi, '');
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < first.length; ) {
    const word = first[i].replace(/[^\w$]/gi, '');
...
function reverseWords(str) {
  ...
  for (let i = 0; i < array.length; i += 1) {
    arrayOfArrays.push(help(array[i]));
  }

  let result = '';
  for (let i = 0; i < arrayOfArrays.length; i += 1) {
    result += `${arrayOfArrays[i].join('')} `;
  }
  result = result.trim();
...
function encodeToRot13(str) {
  ...
  const b = 'NOPQRSTUVWXYZ';
  const c = 'abcdefghijklm';
  const d = 'nopqrstuvwxyz';

  const result = [];
  for (let i = 0; i < str.length; i += 1) {
    const char = str[i];
    if (a.includes(char)) {
      result.push(b[a.indexOf(char)]);
    } else if (b.includes(char)) {
      result.push(a[b.indexOf(char)]);
    } else if (c.includes(char)) {
      result.push(d[c.indexOf(char)]);
    } else if (d.includes(char)) {
      result.push(c[d.indexOf(char)]);
    } else {
      result.push(char);
    }
  }
...
function getCardId(value) {
  ...
  let res = '';
  for (let i = 0; i < deskCards.length; i += 1) {
    if (deskCards[i] === value) {
      res = i;
    }
  }
  return res;
